...
Qe = (X_train @ w * y_train < 0).mean() # доля неверных классификаций
np.random.seed(0)
...
for i in range(N):
    ...
    Qe = lm * Q + (1 - lm) * Qe
    # обратить внимание, что делим на batch_size при расчете градиента
    grad_Q = 1 / batch_size * df(w, X_train[k:index], y_train[k:index])
    w = w - nt * grad_Q
